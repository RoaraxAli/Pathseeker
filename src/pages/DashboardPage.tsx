...
Generated: ${new Date().toLocaleString()} via PathSeeker (Aptech TechWiz 6)`;
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-row font-sans selection:bg-zinc-800 selection:text-white relative">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 xl:w-72 bg-zinc-950/95 backdrop-blur-2xl border-r border-white/[0.08] flex flex-col justify-between transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="p-5 border-b border-white/[0.08]">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => setIsSidebarOpen(false)}
              className="lg:hidden p-1.5 rounded-lg text-zinc-400 hover:text-white hover:bg-white/[0.04] cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          </div>

          <div className="mt-4 flex items-center justify-between px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08]">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-[11px] font-mono uppercase text-zinc-300 font-semibold tracking-wide">
                {role}
              </span>
            </div>
            <span className="text-[10px] font-mono text-zinc-400 font-bold bg-white/5 px-2 py-0.5 rounded-md">
              {profile?.readinessScore || 78}% Ready
            </span>
          </div>
        </div>

        {/* Center: Navigation Menu Items */}
        <div className="flex-1 overflow-y-auto p-3 space-y-1 scrollbar-none text-left">
          <p className="text-[10px] font-mono uppercase text-zinc-500 px-3 py-1 font-semibold tracking-wider">
            Navigation Hub
          </p>
          {[
            { id: 'overview', label: 'Passport Overview', icon: LayoutDashboard, badge: null },
            { id: 'careers', label: 'Career Bank', icon: Search, badge: careers.length || 20 },
            { id: 'quiz', label: 'AI Assessment', icon: BrainCircuit, badge: quizQuestions.length ? `${quizQuestions.length}Q` : null },
            { id: 'multimedia', label: 'Multimedia Center', icon: Video, badge: multimedia.length || 7 },
            { id: 'stories', label: 'Success Stories', icon: BookOpen, badge: stories.length || 6 },
            { id: 'resources', label: 'Resource Library', icon: FileText, badge: resources.length || 5 },
            { id: 'bookmarks', label: 'Sticky Notes & Saved', icon: Bookmark, badge: bookmarks.length || null },
                ...
                {item.badge !== null && (
                  <span
                    className={`text-[10px] font-mono px-2 py-0.5 rounded-full ${
                      isActive ? 'bg-black/10 text-black font-bold' : 'bg-white/5 text-zinc-400'
                    }`}
                  >
                    {item.badge}
                  </span>
          ...
          {role === 'admin' && (
            <div className="pt-3 mt-3 border-t border-white/[0.08]">
              <Link
                to="/admin/dashboard"
                className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-medium text-amber-300 hover:bg-amber-500/10 border border-amber-500/20 transition-all cursor-pointer"
              >
                <div className="flex items-center gap-2.5">
        ...
        <div className="p-4 border-t border-white/[0.08] bg-white/[0.01]">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0 text-left">
              ...
              <p className="text-[10px] text-zinc-500 truncate leading-none mt-0.5 font-mono">
                {profile?.email || 'user@pathseeker.org'}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="flex items-center justify-center gap-1.5 py-2 px-2 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] text-[11px] text-zinc-300 border border-white/10 font-medium transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 py-2 px-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-[11px] text-red-400 border border-red-500/20 font-medium transition-all cursor-pointer"
            >
